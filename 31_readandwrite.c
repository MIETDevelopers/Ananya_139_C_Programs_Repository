...
int main() {
	...
	FILE *fptr;
	int i;
	fptr=fopen("E:\\Softwares\\C_Programs\\Ananya_139_C_Programs_Repository\\031_ReadWriteMarks.txt","wb"); //Opening the file from the location.
	for (i=0;i<1;++i) {
		fflush(stdin);
	...
	fclose(fptr);
	fptr=fopen("E:\\Softwares\\C_Programs\\Ananya_139_C_Programs_Repository\\031_ReadWriteMarks.txt","rb");
